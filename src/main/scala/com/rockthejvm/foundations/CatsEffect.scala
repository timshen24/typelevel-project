...
object CatsEffect extends /*IOApp*/ IOApp.Simple {
  ...
  def evaluateIO[A](io: IO[A]): Unit = {
    ...
    val meaningOfLife =
      io.unsafeRunSync() // but this is not an ideal way. The ideal way is to extend IOApp
...
//  }
  ...
  val manyPrints = for {
    // These two delayedPrint will be performed sequentially. It means you will have to wait two seconds. Wait one second, println then wait another second and then print
    _ <- delayedPrint
    _ <- delayedPrint
  } yield ()

  val manyPrints_v2 = for {
    // These two delayedPrint will be performed in parallel now. It means you will have to wait one seconds. Wait a second and then two numbers will be printed roughly the same time.
  ...
  trait MyMonadCancel[F[_], E] extends MonadError[F, E] {
    trait CancellationFlagResetter {
      def apply[A](
          fa: F[A]
      ): F[A] // You can use this argument to mark certain pieces of your IO in `IO.uncancelable(_ => delayedPrint.onCancel(IO(println("I'm cancelled!")))).start` in `ignoredCancellation` to be uncancelable and some not to be cancelable.
    }
  ...
  trait MySync[F[_]] extends MonadCancel[F, Throwable] with Defer[F] {
    ...
    def blocking[A](
        expression: => A
    ): F[A] // blocking will run that particular effect that we obtain at the end on a dedicated thread pool so that we don't starve our original thread pool for effects for threads.
  }
  ...
  trait MyAsync[F[_]] extends Sync[F] with Temporal[F] {
    ...
    ): F[A] // too complicated. See cats-effect lesson
  }
...
//  override def run: IO[Unit] = ignoredCancellation // It will print "cancelling fiber" but the cancel operation is ignored. Number will be printed as well.
  override def run: IO[Unit] =
    copyFileEffect // It will print "cancelling fiber" but the cancel operation is ignored. Number will be printed as well.
}
